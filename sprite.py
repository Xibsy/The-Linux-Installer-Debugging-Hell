...
class Sprite:
    ...
    def load_raw_image(cls, path: Path | str, shape: float = 1., pivot: Vector2Int = Vector2Int.zero()) -> "Sprite":
        ...
        return cls(image, Vector2(*image.size) * shape, pivot)
    ...
    def blit_at(self, position: Vector2Int, angle: float=0.0) -> None:
        screen_position = position - self._pivot
        arcade.draw_texture_rect(self._image, arcade.rect.XYWH(*screen_position.tuple, *self.shape.tuple), angle=angle)
